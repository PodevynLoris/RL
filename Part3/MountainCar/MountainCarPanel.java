...
public class MountainCarPanel extends JPanel {
    ...
    public void render(double[] state) {
        ...
        carX = getXFor(getPositionFromIndex((int) state[2]));
        carY = getYFor(getPositionFromIndex((int) state[2]));
        this.repaint();
    }

    public double getPositionFromIndex(int positionIndex) {
        double positionStep = (MountainCarEnv.MAX_POS - MountainCarEnv.MIN_POS) / MountainCarEnv.numberOfPositionBins;
        // Getting the midpoint of the interval for the given index
        return MountainCarEnv.MIN_POS + positionStep * positionIndex + positionStep / 2;
    }
